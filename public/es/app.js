  ...
  $("deck-form").addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch("/api/decks", {
        ...
        body: JSON.stringify({ url, gradeBand, theme, pastedText, lang: "es", website: ($("hp-website") || { value: "" }).value, turnstileToken: window.__tsToken || "" }),
      });
      ...
      stopLoading();
      show("home");
      errEl.textContent = "No pudimos conectar con el servidor. Revisa tu internet e inténtalo de nuevo.";
      errEl.classList.remove("hidden");
    }
  });
