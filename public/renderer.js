...
(function () {
    ...
    switch (cardSpec.type) {
      ...
      case "quiz": {
        ...
        (s.quiz || []).forEach((q) => {
          ...
          li.append(
            el("div", "", "❓ " + (q.q || "")),
            el("div", "quiz-hint", "tap to reveal the answer"),
            el("div", "answer", "✅ " + (q.a || ""))
          );
  ...
  window.renderDeck = function renderDeck(deck, mount) {
    ...
    if (deck.authorName) hero.append(el("span", "byline", "made by " + deck.authorName));
    root.append(hero);
    ...
    if (deck.videoId) {
      ...
      const meta = el("div");
      meta.append(el("div", "", "From the video: " + (deck.videoTitle || "")));
      const a = el("a", "", "Watch the full video on YouTube →");
