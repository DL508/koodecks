...
(function () {
  ...
  $("deck-form").addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch("/api/decks", {
        ...
        body: JSON.stringify({ url, gradeBand, theme, pastedText, website: ($("hp-website") || { value: "" }).value, turnstileToken: window.__tsToken || "" }),
      });
      ...
      stopLoading();
      show("home");
      errEl.textContent = "Couldn't reach the server. Check your internet and try again.";
      errEl.classList.remove("hidden");
    }
  });
  ...
  async function saveImage() {
    ...
    try {
      ...
      const file = new File([blob], (currentDeck.slug || "brightdeck") + ".png", { type: "image/png" });
  ...
  fetch("/api/auth/me").then((r) => r.json()).then((d) => {
    ...
    if (!link) return;
    if (d.user) {
      const left = d.user.plan === "paid" ? "Unlimited" : (d.user.remainingToday + " decks left today");
