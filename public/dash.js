...
(function () {
  ...
  async function loadHome() {
    ...
    try {
      ...
      $("daily-title").textContent = `Today: ${d.title} · same 5 questions for everyone`;
      $("daily-btn").onclick = () => start({ ...d, mode: "daily" });
    } catch { $("daily-title").textContent = "Couldn't load today's dash — check your internet."; }
    try {
      ...
      packs.forEach((p) => {
        ...
        b.className = "pack";
        b.innerHTML = `<span class="pe">${p.emoji}</span><b>${p.title}</b><small>${p.subject} · ${p.count} questions</small>`;
        b.onclick = async () => {
    ...
    if (slug) {
      try {
        ...
        $("challenge-text").textContent = `⚔️ ${challenge.name} scored ${challenge.score} ${challenge.emojiRow}. Beat it?`;
  ...
  function finish() {
    ...
    $("done-headline").textContent =
      rights === 5 ? "Perfect dash!" : rights >= 3 ? "Nice dash!" : "Good try — dash again!";
    ...
    if (game.target) {
      sub += game.score > game.target.score
        ? ` · You beat ${game.target.name} (${game.target.score})! 👑`
        : ` · ${game.target.name} still leads with ${game.target.score}. Rematch?`;
    }
    if (game.score > prev && prev > 0) sub += " · New personal best!";
  ...
  async function share() {
    ...
    const label = game.packId === "daily" ? `Daily Dash — ${game.date}` : `Daily Dash · ${game.title}`;
    const text = `${label} ${game.emoji}\n${game.score} pts ${game.row.join("")}\nBeat me 👉 ${url}`;
    try {
      if (navigator.share) { await navigator.share({ text }); }
      else { await navigator.clipboard.writeText(text); toast("Score card copied! Paste it anywhere 📋"); }
    } catch { toast(text); }
    $("share-btn").disabled = false;
  }

  function promptName() {
    const n = (prompt("Your name for the challenge? (or leave blank)") || "").slice(0, 24);
