...
(async () => {
  ...
  await t("A", "app page has honeypot + KooDeck brand, and NO Turnstile (scoped to login/signup)", async () => {
    const html = await (await U.get("/app")).text();
    assert.ok(/id="hp-website"/.test(html), "no honeypot on app");
    assert.ok(!/data-tskey/.test(html), "Turnstile should NOT be on the app page anymore");
