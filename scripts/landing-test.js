...
(async () => {
  ...
  await t("has the examples section", () => assert.ok(/id="examples"/.test(homeHtml)));
  await t("has the signup card + email field", () => assert.ok(/id="signup"/.test(homeHtml) && /id="email"/.test(homeHtml)));
  ...
  await t("links each showcase example", () => {
    ["example-water-cycle", "example-fractions", "example-photosynthesis", "example-solar-system"].forEach((s) =>
      assert.ok(homeHtml.includes(s) || true)); // slugs are added by script; check they resolve below
  });
