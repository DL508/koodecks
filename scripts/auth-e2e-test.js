...
console.log = function (...a) {
  const s = a.join(" ");
  const m = s.match(/reset\?token=([^\s)]+)/); // stop at whitespace or ")"
  if (m) { capturedToken = decodeURIComponent(m[1]); capturedLink = s.slice(s.indexOf("http")); }
...
setTimeout(async () => {
  ...
  t("request-reset NO LONGER requires Turnstile (scoped to signup/login) — still returns ok", (await post("/api/auth/request-reset", { email: "teach@school.edu" })).status === 200);
  ...
  // 7) The no-email admin CLI (scripts/reset-link.js) produces a token the flow accepts.
  const { execFileSync } = require("child_process");
  const path = require("path");
  await realFetch(B + "/api/auth/register", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: "cli@school.edu", password: "password123", turnstileToken: "good-token" }) });
  const out = execFileSync("node", [path.join(__dirname, "reset-link.js"), "cli@school.edu"],
    { env: { ...process.env, SITE_URL: B }, encoding: "utf8" });
  const cliToken = (out.match(/reset\?token=([^\s)]+)/) || [])[1];
  t("reset-link CLI prints a reset URL", !!cliToken);
  const cliReset = await post("/api/auth/reset", { token: decodeURIComponent(cliToken || ""), password: "cliNewPass123" });
  t("CLI-generated token is accepted by the reset flow (no email needed)", cliReset.status === 200);
  t("password set via CLI link works for login", (await post("/api/auth/login", { email: "cli@school.edu", password: "cliNewPass123" })).status === 200);
