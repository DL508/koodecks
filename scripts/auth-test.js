...
(async () => {
  ...
  await t("free user starts with 1 deck remaining", () => {
    assert.equal(auth.remainingToday(auth.getUser("unit@school.edu")), 1);
  });
  ...
  console.log("\n3) Freemium: accounts required, 1 deck/day free");
  await t("deck creation WITHOUT an account is blocked (401 needsAuth)", async () => {
    // Only registered teachers/parents create; students view/remix without an account.
    ...
    assert.equal(r.status, 401, "anonymous creation should be blocked, got " + r.status);
    assert.equal((await r.json()).needsAuth, true);
  });
  await t("free user can make 1 deck (the daily allowance)", async () => {
    const r = await A.post("/api/decks", { pastedText: longText, gradeBand: "35", theme: "space" });
    assert.equal(r.status, 200, "first deck should succeed");
    const d = await r.json();
    assert.equal(d.deck.ownerEmail, "teacher@school.edu", "deck should be owned by the creator");
  });
  await t("the 2nd deck same day is blocked with needsUpgrade (402)", async () => {
    const r = await A.post("/api/decks", { pastedText: longText });
    assert.equal(r.status, 402);
    const d = await r.json();
    assert.equal(d.needsUpgrade, true);
  });
  await t("registered user finds their own deck in /api/my/decks", async () => {
    const d = await (await A.get("/api/my/decks")).json();
    assert.equal(d.decks.length, 1);
    assert.equal(d.decks[0].ownerEmail, "teacher@school.edu");
  });
