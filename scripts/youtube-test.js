...
(async()=>{
  ...
  // Case 7: a HANGING fetch must time out (not hang the request forever).
  await withMock(() => new Promise(() => {}), async () => {
    const start = Date.now();
    let code = null;
    try { await yt.fetchTranscript("vidHANG0007", "en"); } catch (e) { code = e.message; }
    const elapsed = Date.now() - start;
    t("hanging fetch times out fast (<25s) as TRANSCRIPT_FETCH_FAILED", elapsed < 25000 && code === "TRANSCRIPT_FETCH_FAILED");
  });
