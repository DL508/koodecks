...
(async () => {
  ...
  await t("Turnstile site key injected into the login/signup page ONLY", async () => {
    // Turnstile is scoped to signup/login. It must be present there...
    const login = await (await realFetch(B + "/login")).text();
    assert.ok(login.includes('data-tskey="1x-test-site-key"'), "login should carry the Turnstile key");
    // ...and NOT on the landing or app pages anymore.
    const l = await home.text();
    assert.ok(!/data-tskey/.test(l), "landing should have no Turnstile");
    const a = await (await realFetch(B + "/app")).text();
    assert.ok(!/data-tskey/.test(a), "app should have no Turnstile");
  });
  await t("honeypot fields still present on app, login, and share pages", async () => {
    const a = await (await realFetch(B + "/app")).text();
    assert.ok(/id="hp-website"/.test(a), "app honeypot");
    const login = await (await realFetch(B + "/login")).text();
    assert.ok(/id="hp-website"/.test(login), "login honeypot");
    const sh = fs.readFileSync(__dirname + "/../public/share.html", "utf8");
    assert.ok(/id="hp-website"/.test(sh), "share honeypot");
  });

  console.log("\n3) Turnstile enforcement (feature ON) — scoped to signup/register only");
  await t("register with a good token → account created", async () => {
    const r = await post("/api/auth/register", { email: "real@school.edu", password: "password123", turnstileToken: "good-token" });
    assert.equal(r.status, 200);
  });
  await t("register with a bad token → 403 security-check message", async () => {
    const r = await post("/api/auth/register", { email: "bot@spam.io", password: "password123", turnstileToken: "bad-token" }, { "x-forwarded-for": "20.0.0.2" });
    assert.equal(r.status, 403);
    assert.ok(/security check/i.test((await r.json()).error));
  });
  await t("register with NO token → 403 (token required when enabled)", async () => {
    const r = await post("/api/auth/register", { email: "bot2@spam.io", password: "password123" }, { "x-forwarded-for": "20.0.0.3" });
    assert.equal(r.status, 403);
  });
  await t("deck creation NO LONGER blocked by Turnstile (scoped out)", async () => {
    // This suite runs with KOO_TEST_BYPASS_AUTH, so the auth gate is skipped. The
    // point here is that Turnstile no longer blocks: the request is NOT a 403
    // security-check rejection (it proceeds to the pipeline, which may 200/502).
    const r = await post("/api/decks", { pastedText: "x".repeat(400) }, { "x-forwarded-for": "20.0.0.4" });
    assert.notEqual(r.status, 403, "deck creation should not be Turnstile-blocked anymore");
  });
  await t("email signup endpoint NO LONGER requires Turnstile (scoped out)", async () => {
    const r = await post("/api/signups", { email: "ok@school.edu", role: "teacher" }, { "x-forwarded-for": "20.0.0.5" });
    assert.equal(r.status, 200, "signup should succeed without a token now, got " + r.status);
  });
