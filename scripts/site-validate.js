...
(async () => {
  ...
  await t(1, "landing loads with hero, signup, and how-it-works", () => {
    assert.ok(/deck kids/i.test(home));
    assert.ok(/id="signup"/.test(home) && /id="email"/.test(home));
    assert.ok(/id="how"/.test(home));
  });
  await t(1, "landing promotes Daily Dash (nav + banner + footer)", () => {
    assert.ok((home.match(/href="\/play"/g) || []).length >= 3);
    assert.ok(/Daily Dash/.test(home));
  });
  await t(1, "all four example decks resolve from the landing", async () => {
    for (const s of ["example-water-cycle", "example-fractions", "example-photosynthesis", "example-solar-system"]) {
      assert.equal((await get("/d/" + s)).status, 200, s);
    }
  });
  await t(1, "signup accepts a valid teacher email", async () => {
