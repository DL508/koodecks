...
// ---------- password reset ----------
// Token-based flow. A one-time, time-limited token is stored (hashed) on the
// user record. The raw token goes into a reset link. Tokens expire in 1 hour
// and are single-use. We never reveal whether an email exists.
const RESET_TTL_MS = 60 * 60 * 1000; // 1 hour
function createResetToken(email) {
  const u = getUser(email);
  if (!u) return null; // caller still returns a generic success (no account enumeration)
  const raw = crypto.randomBytes(24).toString("base64url");
  const tokenHash = crypto.createHash("sha256").update(raw).digest("hex");
  u.reset = { tokenHash, expires: Date.now() + RESET_TTL_MS };
  saveUser(u);
  return raw; // raw token is delivered via the reset link; only the hash is stored
}
function resetPassword(rawToken, newPassword) {
  ...
  // Find the user whose stored reset hash matches (scan the small user store).
  let match = null;
  try {
    for (const f of fs.readdirSync(USERS_DIR)) {
      if (!f.endsWith(".json")) continue;
      let u; try { u = JSON.parse(fs.readFileSync(path.join(USERS_DIR, f), "utf8")); } catch { continue; }
      if (u && u.reset && u.reset.tokenHash === tokenHash) { match = u; break; }
    }
  } catch { /* no users dir yet */ }
  if (!match) return { error: "This reset link is invalid or has already been used." };
  if (!match.reset.expires || match.reset.expires < Date.now()) {
    delete match.reset; saveUser(match);
    return { error: "This reset link has expired. Please request a new one." };
  }
  match.passwordHash = hashPassword(newPassword);
  delete match.reset; // single-use
  saveUser(match);
