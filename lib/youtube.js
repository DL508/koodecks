...
async function fetchTranscript(videoId, lang) {
  // Strategy 1: the library (fast path). Treat ANY throw/empty as "try the next
  // strategy" rather than immediately declaring no captions.
  try {
    const t = await viaLibrary(videoId);
  ...
  return await viaInnertube(videoId, lang);
}
