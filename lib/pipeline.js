...
async function callClaude(system, user, maxTokens) {
  ...
  try {
    res = await fetch(API_URL, {
      ...
      // Never hang the request forever if the AI is slow/unreachable — fail
      // cleanly so the server returns JSON instead of the connection dropping.
      signal: AbortSignal.timeout(45000),
    });
  } catch (e) {
    const err = new Error("CLAUDE_API_TIMEOUT");
